...
int checkplace_tetromino(board b, int r,int c, tetromino t) {
    if (t.points[0]+r < 0 || t.points[1]+c < 0 || t.points[0]+r > b.lignes -1 || t.points[1]+c > b.colonnes -1) {
        return 0 ; }
    else {
        for (int j = 0 ; j < longueur_liste(b.plateau) ; j++) {
            tetromino tt = accéder_element(b.plateau,j) ;
            for (int k = 0 ; k < 4 ; k++) {
                if (t.points[0]+r == tt.points[2*k] && t.points[1]+c == tt.points[2*k+1]) {
                    return 0 ; } } } } 
    return 1 ; }
...
int place_tetromino(board * b,int r,int c, tetromino t ) {
    t.points[0] = t.points[0] + r ;
    t.points[1] = t.points[1] + c ; 
...
void display_board(board b) {
    ...
    for (int i = 0 ; i < b.taille_sac; i ++) {
        printf("%d",accéder_element(b.sac,i).score) ;
        printf(" ") ; }
    printf("\n") ;
    printf("Plateau :\n") ;
...
tetromino create_trivial_piece() {
    tetromino t ;
    t.score = rand()% 3 +1 ;
    t.couleur = compteur ;
    compteur = (compteur + 1) % 7;
...
int corner_fit_2(board * b) {
    ...
    int meilleur_position[2] = {-10, -10}; // ligne, colonne
        ...
        for (int j = 0 ; j < b->colonnes ; j ++) { // Pour chaque colonne
            if (checkplace_tetromino(*b,i,j,t)) {
                for (int l = 0 ; l < 4 ; l++) { // Pour chaque coin
                    int distance = abs(t.points[0] + i - coins[l][0]) + abs(t.points[1] + j - coins[l][1]) ;
...
int main() {
    ...
    #endif
    board b = create_board(6,6,3) ;
    for (int k = 0 ; k < b.taille_sac ; k++) {
        add_tetromino_to_bag(create_trivial_piece(),&b) ; }
    ...
    for (int i = 3 ; i < 21 ; i++) { // Pour chaque taille de plateau, on fait 10 tests puis on fait une moyenne des temps et des scores
        ...
        for (int j = 0 ; j < 10 ; j++) {
            ...
            remplissage = remplissage + longueur_liste(b.plateau) ;
            free_board(b) ; }
...
// gcc code.c -Ofast -o mon_programme -Wall -Wextra -pedantic || ./mon_programme